---
name: coworker-debug-issue-debug
triggers:
  - debug this bug
  - investigate error
  - find root cause
...
description: |
  Scientific debugging — hypothesis, test, confirm, fix. No guessing. Use the
  strongest model available. Forms ranked hypotheses and tests them in order.
services:
  category: coworker-debug
when_to_use: |
  When a bug exists and the cause is unknown. Form hypothesis, test it, confirm,
  then fix. Do not propose fixes without confirmed root cause.
when_not_to_use: |
  Skip if the bug cause is already known — go straight to the fix.
version: 1.0.0
---
